...
main(int argc, char *argv[]) {
    ...
    mzout.open("meanN-FFU.txt", ios::out);
    iterout.open("iter-FFU.txt", ios::out);
...
//            dCTM += dCTM0;
        ...
        if ((ntimestep + 1) % 100 == 0) {
            ...
            for(auto i:range(nsite))
                writeToFile(format("FFU_As_%d_%d_%d", bondD,ntimestep+1,i), ctm.tensorAs().at(i));

            for(auto i:range(2*nsite))
                writeToFile(format("FFU_Links_%d_%d_%d", bondD,ntimestep+1,i), ctm.linkAs().at(i));
...
//                writeToFile(format("Links_%d_%d", bondD, ntimestep+1), ctm.linkAs());
    ...
    for (auto i:range(nsite)) {
        ...
        auto col = i % nx;
        mz.push_back(ctm.exp_site(sites.op("N", i + 1), i));
        cout << "FFU results: mz at (line,col) = ( "
    ...
    for (auto i:range(2 * nsite)) {
        ...
        energy.push_back(ctm.exp_bond(hh, i));
        cout << "FFU results: energy at bond " << i << " ) equals " << energy[i] << endl;
    }
    energytot = accumulate(energy.begin(), energy.end(), 0.0) / nsite;

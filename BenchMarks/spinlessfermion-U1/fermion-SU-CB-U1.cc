    ...
    for (auto i:range(nSite)) {
        linksG.push_back(
                array<IQIndex, 2>{{IQIndex(nameint("UpIn", i),
                                           Index("q0", 1, Link), QN("Nf", 0),
                                           Index("q1", 2, Link), QN("Nf", 1),
                                           Index("q2", 1, Link), QN("Nf", 2),In),
                                          IQIndex(nameint("UpOut", i),
                                                  Index("q0", 1, Link), QN("Nf", 0),
                                                  Index("q1", 2, Link), QN("Nf", 1),
                                                  Index("q2", 1, Link), QN("Nf", 2))}});
        linksG.push_back(
                array<IQIndex, 2>{{IQIndex(nameint("LeftIn", i),
                                           Index("q0", 1, Link), QN("Nf", 0),
                                           Index("q1", 2, Link), QN("Nf", 1),
                                           Index("q1", 1, Link), QN("Nf", 2), In),
                                          IQIndex(nameint("LeftOut", i),
                                                  Index("q0", 1, Link), QN("Nf", 0),
                                                  Index("q1", 2, Link), QN("Nf", 1),
                                                  Index("q1", 1, Link), QN("Nf", 2))}});
    }
    Ls.clear();
    for (auto &I:linksG) {
...
//            ctm.randEnv();
            ...
            writeToFile(format("SUCB_SymInd_%d_%d",nrepeat,bondD),ctm.symInd());
            for(auto i:range(nsite))
                writeToFile(format("SUCB_As_%d_%d_%d",nrepeat,bondD,i),As[i]);

            for(auto i:range(2*nsite))
                writeToFile(format("SUCB_Links_%d_%d_%d",nrepeat,bondD,i),Links[i]);
/*
            if(dctm<dctmOld){
                for(auto i:range(nsite))
                    writeToFile(format("SU_As_%d_%d", bondD, i),As[i]);
                    writeToFile(format("SU_Links_%d_%d",bondD,i),Links[i]);
            }
            dctmOld=dctm;
            */
